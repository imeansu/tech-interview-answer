...
- OSI 7계층은 **네트워크 통신**을 구성하는 **요소**들을 7개의 **계층**으로 **표준화** 한 것
- 표준화의 장점은 통신이 **일어나는 과정**을 **단계별로 파악**할 수 있어, **문제**가 발생하면 **해당 문제를 해결하기 용이**해짐
1. 물리 계층
    - 실제로 장치들을 연결하기 위한 물리적인 사항을 저의
    - 주로 전기적 및 기계적인 특성으로 통신 케이블로 데이터를 전송하는 기능
    - 사용되는 단위는 bit 이며 전기적으로 on, off를 나타낸다
    - 케이블, 리피터, 허브
2. 데이터 링크 계층
    - 두 지점 간의 신뢰성 있는 전송을 보장
    - 물리계층을 통하여 송수신 되는 정보의 흐름과 오류를 관리
    - MAC 주소를 가지고 통신, 2계층 까지만 있으면 실제 네트워크 통신이 가능함(두 컴퓨터를 케이블로 연결)
    - 사용 되는 단위는 프레임
3. 네트워크 계층
    - 여러 개의 지점을 거칠 때 경로를 찾아줌
    - 가장 중요한 목적은 데이터를 목적지까지 안전하고 빠르게 전달하는 것, 라우팅
    - 경로배정, 랜상 상호 네트워크 가능
    - 사용 되는 단위는 패킷
    - 라우터, L3 스위치
    - IP를 사용하기 위해 네트워크 계층이 필요
        - 2계층 Mac 주소 만으로는 전세계의 모든 컴퓨터가 연결될 수 없음
4. 전송 계층
    - 양 끝단의 사용자들이 송수신에 있어서 신뢰성을 보장
    - 정보를 분할하고 다시 합치는 과정 담당
    - 통신 과정에 있어 오류제어와 흐름제어를 한다
    - 사용 되는 단위는 세그먼트?
    - TCP, UDP - Port (응용프로그램 사용하기 위한 번호)
5. 세션 계층 
    - 양 끝단의 프로세스가 통신을 관리하기 위한 방법 제공
6. 표현 계층
    - 인코딩 및 데이터의 형식 차이를 조절
    - 다양한 데이터의 포멧을 일정한 포멧으로 변환, 압축, 암호화, 복호화 작업
7. 응용 계층
    - 응용프로그램과 사용자 사이의 인터페이스를 제공

### TCP/IP 4계층

- 어딘가 사양이 변경되었을 때, 전체를 바꾸지 않아도 됨 / 서로 간의 간섭을 최소화하여 사용의 편리성을 높힘
- 계층화로 사양이 변경된 해당 계층만 바꾸면 됨. 각 계층은 계층이 연결되어 있는 부분만 결정되어 있어, 각 계층의 내부는 자유롭게 설계
...
해당 IP를 그 IP 주소에 맞는 물리적인 주소 즉, MAC 주소를 가지고 오는 프로토콜
> 

### ARP 동작 원리

- 송신자는 목적지 물리주소가 필요하므로, 물리주소 요청을 위한 ARP 요청 패킷을 브로드 캐스트로 전송
...
1. 송신자는 목적지 IP Address 를 지정해 패킷 송신
2. IP 프로토콜이 ARP 프로토콜에게 ARP Request 메시지를 생성하도록 요청
    
    = ARP 요청 메시지 (송신자 물리주소, 송신자 IP 주소, 00-00-00-00-00, 수신자 IP 주소)
    
3. 메시지는 2계층으로 전다로디고 이더넷 프레임으로 Encapsulation 됨
    
    = 송신자 물리주소를 발신지 주소, 수신자 물리주소를 브로드캐스트 주소로 지정
    
4. 모든 호스트와 라우터는 프레임을 수신 후 자신의 ARP 프로토콜에게 전달
5. 목적지 IP Address 가 일치하는 시스템은 자신의 물리주소를 포함하고 있는 ARP Reply 메시지를 보냄
    
    = 자신의 물리주소를 포함하는 응답 메시지
    
6. 최초 송신 측은 지정한 IP Address 에 대응하는 물리주소를 획득

cf) ARP 요청은 브로드캐스트, ARP 응답은 유니캐스트

### ARP request 패킷 분석

- Destination : Ethernet Frame 에서 사용하는 항목으로, 물리주소 (MAC 주소) 기록됨
- opcode : ARP request 요청임을 알 수 있음 (request or reply 2개 뿐임)
- Target MAC address : 해당 목적지의 MAC 주소를 기록함. 아직 목적지 MAC 주소를 모르므로 0으로 값을 채워서 보냄

### ARP reply 패킷 분석

- source : 송신자의 물리주소(MAC 주소)를 Ethernet 의 frame에 적어준다
- opcode : ARP Reply 패킷임을 알려줌
- sender MAC address : 요청 받은 호스트에서 MAC 정보를 (arp request를 보낸) 호스트에게 알려준다
